...
class CBSSolverDG(object):
    ...
    def find_solution(self, disjoint=True):
        ...
        dependency_graph = build_dependency_graph(self.my_map, list(range(0, self.num_of_agents)), self.starts,
                                                  self.goals, root['collisions'], root['constraints'], root['paths'],
                                                  self.cardinals_list,
                                                  self.cache)

        V, E = dependency_graph
        root['edges'] = E
        hvaluedict = dict()
        edgedict = dict()
        edgedict[root['id']] = E
        # computing h-value
        for k in range(0, self.num_of_agents + 1):
            if does_mvc_exist(V, E, k):
                root['h-value'] = k
                root['h2applied'] = True
        ...
        while len(self.open_list) > 0:
            p = self.pop_node()
            # LAZY Approach for h2
            if p['h2applied'] is False:
                dg = build_dependency_graph_from_partial(self.my_map, list(range(0, self.num_of_agents)), self.starts,
                ...
                vertices, edges = dg
                edgedict[p['id']] = edges
                for k in range(max(hvaluedict[p['parent']] - 1, 0), hvaluedict[p['parent']] + 2):
                    if does_mvc_exist(vertices, edges, k):
                        p['h-value'] = k
                        p['h2applied'] = True
                        # max_h = max(max_h,k)
                        hvaluedict[p['id']] = k
                        break
                # if can not find then just pass 0
                if p['h2applied'] is False:
                    p['h-value'] = 0
                    p['h2applied'] = True
                    # max_h = max(max_h,k)
            ...
            for constraint in constraints:
                ...
                if agent_path is not None:
                    ...
                    if str(q['paths']) not in self.closed_list:
                        ...
                        # Lazy approach, apply on h1, we will compute h2 later
                        q['h-value'] = 0
                        q['h2applied'] = False
