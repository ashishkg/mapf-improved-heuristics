...
def disjoint_splitting(collision):
    ...
    constraints = []
    # instead of positive_index = random.randint(0, 1) , i used shuffle

    # print("print inside standard splitting")
    # print(collision)
    if len(collision["loc"]) > 1:
        # handle edge vertex
        first_constraint = {
...
def extract_constraints_violated(agent, constraint):
    constraints = []
...
class CBSSolverDisjoint(object):
    ...
    def find_solution(self, disjoint=True):
        ...
        while len(self.open_list) > 0:
            ...
            if len(p["collisions"])==0:
                ...
                #return [p['paths'], self.num_of_expanded, self.num_of_generated, str("{:.9f}".format(
            ...
            for constraint in constraints:
                ...
                agents_violated = paths_violate_constraint(constraint, q['paths'])
                for a in agents_violated:
                        constaints_violated = extract_constraints_violated(a,constraint)
                        for c in constaints_violated:
                            if c not in q['constraints']:
                                q['constraints'] = q['constraints'] + [c]

                for a in agents_violated:
                        ares = a_star(self.my_map, self.starts[a], self.goals[a], self.heuristics[a],
                              a, q['constraints'])
                        if ares is None:
                            valid = False
                        else:
                ...
                if valid and agent_path is not None:
        ...
        return [root['paths'], self.num_of_expanded, self.num_of_generated,str("CPU time (s):    {:.9f}".format(timer.time() - self.start_time))]
